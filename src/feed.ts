...
async function resolveMp3(relinker: string) {
  const res = await fetchT(relinker, { method: 'HEAD', redirect: 'follow' })
  let url = res.url
...
async function buildFeed(program: string, forceRefresh: boolean = false) {
  ...
  const feed = new Feed({
    ...
    updated: new Date(),
    generator: 'https://github.com/Ric-82/raiplaysound-feed',
    feed: `https://ric-82.github.io/raiplaysound-feed/rss/${program}.xml`,
    podcast: true
  })
